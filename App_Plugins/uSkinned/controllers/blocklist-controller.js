(function () {
    ...
    angular
        .module("umbraco")
        .component("usnBlockListPropertyEditor", {
            templateUrl: "/App_Plugins/uSkinned/views/usn-block-list-property-editor.html",
            controller: USNBlockListController,
            controllerAs: "vm",
            bindings: {
                model: "="
            },
    ...
    function USNBlockListController($scope, $timeout, $interpolate, editorService, clipboardService, localizationService, overlayService, blockEditorService, udiService, serverValidationManager, angularHelper, eventsService, $attrs, editorState, contentResource, $http, $compile, $document, $routeParams, notificationsService) {
        ...
        var currentContent = editorState.getCurrent();
        vm.isNew = false;

        if (currentContent && currentContent.id === 0) {
            vm.isNew = true;
        };
        ...
        vm.$onInit = function() {
            ...
            enablePreview = vm.model.config.enablePreview;

            if ($routeParams.section === "usnContentTemplatesSection") {
                enablePreview = false;
            }
        ...
        vm.culture = $scope.$parent.$parent.model.culture;
        ...
        function requestShowCreate(createIndex, mouseEvent) {
            ...
            if (vm.isNew) {
                ...
                if (enablePreview === true) {
                    ...
                    var currentNode = editorState.current;

                    var languageVariant = currentNode.variants.find(function (variant) {
                        return variant.language.culture === vm.culture;
                    });

                    if (languageVariant) {
                        // Flag the variant for saving
                        languageVariant.save = true;
                    }

                    var minDisplayTime = 500; // Minimum time in milliseconds (e.g., 1 second)
                    var startTime = Date.now(); // Record when the saving message is shown

                    displaySaving()
                        .then(function (savingTemplate) {
                            return contentResource.save(currentNode, false, [], false)
                                .then(function () {
                                    var elapsedTime = Date.now() - startTime;
                                    var remainingTime = Math.max(minDisplayTime - elapsedTime, 0);

                                    $timeout(function () {
                                        removeSaving(savingTemplate);
                                ...
                                .catch(function (error) {
                                    removeSaving(savingTemplate); // Remove saving indicator on error
                                    notificationsService.warning("Unable to save page for preview.", "Save the page to see and fix your validation error before adding or editing blocks.");
                                });
                        })
                        .catch(function (error) {
                            console.error('Error loading saving template:', error);
                        });
                }
                else {
                    if (vm.availableBlockTypes.length === 1) {
        ...
        function displaySaving() {
            ...
            // Check if saving template is already displayed to avoid duplicates
            if (angular.element($document[0].getElementById('USNSaving')).length > 0) {
                return Promise.resolve(null);
            }

            return $http.get('/App_Plugins/uSkinned/views/visual-editor-saving.html').then(
                function (response) {
                    // Success callback
                    var template = response.data; // HTML content as a string

                    //Compile the template with the scope
                    var savingTemplate = $compile(template)($scope);

                    // Append after the <body> tag
                    angular.element($document[0].body).append(savingTemplate);
        ...
        function removeSaving(savingTemplate) {

            // Handle both specific template removal and general cleanup
            if (savingTemplate && savingTemplate.remove) {
                savingTemplate.remove(); // Remove specific element from DOM
            } else {
                // Fallback: remove by ID
                var existingOverlay = $document[0].getElementById('USNSaving');
                if (existingOverlay) {
                    angular.element(existingOverlay).remove();
                }
            }
